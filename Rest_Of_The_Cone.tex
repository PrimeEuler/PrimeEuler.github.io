...
\begin{finding}[Retracted as stated; see Section~\ref{sec:secularpositivity}]\label{find:Rminus}
An earlier version of this note claimed $\langle\dot w\rangle=\langle\dot R_-^2\rangle\geq0$ in every case tested, based on a sweep of $30{,}800$ points finding a minimum of $2\times10^{-10}$. This claim is false: a genuine, narrow negative pocket exists at extreme eccentricity ($e\gtrsim0.9996$) combined with intermediate-to-high bath temperature, missed by that sweep's resolution. What survives, and is now established on much firmer ground than the original numerical claim, is a \emph{secular} (orbit-averaged) positivity statement --- see Section~\ref{sec:secularpositivity} for the corrected picture, including explicit analytic regions where the pointwise statement \emph{is} true, a proof that the circular orbit is a strict local secular minimum, and the precise, now very small, remaining domain of open status.
\end{finding}

Unlike $X^2,Y^2$, the secular version of this is a clean, one-sided statement about a single quantity, not a joint condition on two.
...
\end{equation}
Near apoapsis, $\langle\dot u\rangle$ and $\langle\dot w\rangle$ both grow large (driven by $1/v$-type factors as $v\to0$) while $\langle\dot u\rangle+\langle\dot w\rangle=\langle\dot T^2\rangle$ (Proposition~\ref{prop:identity}) does not grow at the same rate, so the surviving leading piece is $\tfrac{Y}{2}\langle\dot w\rangle$. Away from apoapsis, this leading piece shrinks and the covariance term $\operatorname{Var}(T)-\operatorname{Var}(Y)$, which is not sign-definite (found strongly negative, e.g.\ $-14.5$ against a leading piece of only $2.1$, at $e=0.95,\sigma=0.05,\theta=175°$), can dominate and reverse the sign.
\end{proposition}

\begin{remark}[What this is and is not, updated]
This identifies \emph{why} the handoff of Finding~\ref{find:X2fails} occurs --- a competition between the leading piece $\tfrac{Y}{2}\langle\dot w\rangle$ and a term of unbounded sign --- and isolates the entire remaining difficulty of Finding~\ref{find:neveroverlap} into bounding $\operatorname{Var}(T)-\operatorname{Var}(Y)$ against it. An earlier version of this remark asserted the leading piece was "unconditionally non-negative by Finding~\ref{find:Rminus}"; since that finding is now known false as a pointwise statement (Section~\ref{sec:secularpositivity}), this specific assertion no longer holds without qualification. What remains true, established there, is that $\langle\dot w\rangle$ is non-negative on explicit hot-bath ($\Theta\leq\Theta_0$) and cold-bath ($\Theta\geq(2+q)/4q$) regions, and pointwise for all bath temperatures whenever $e\lesssim0.983$; the leading piece here is unconditionally non-negative on those regions, and unresolved (though secularly safe) only in the narrow remaining wedge. It is not a proof of Finding~\ref{find:neveroverlap} in general: no bound on the covariance term is established here.
\end{remark}
...
For equal masses,
\begin{equation}
\langle\dot w\rangle = \underbrace{\frac{8T(2T-Y)}{\sqrt\pi\,\mu v}\,\chi e^{-\chi^2}}_{\geq0\text{ unconditionally}} \;+\; \Psi(\chi)\cdot\left[\frac{8T^3v}{\chi^2\mu^2}-\frac{3T^2Yv}{\chi^2\mu^2}-\frac{2T^2}{\chi^2\mu v}-\frac{TY}{\chi^2\mu v}+\frac{4Y}{v^3}\right]. \tag{5}
\end{equation}
\end{proposition}

\begin{proof}
Direct computation of $w_h,w_\ell,w_{hh},w_{h\ell}$ (with $w_{\ell\ell}=0$), combined with the $(h,\ell)$ drift and covariance structure of Corollary~\ref{cor:general_sde}, then collected by the coefficient of the diffusion tensor's $D_\perp$, $D_\parallel$, and friction-magnitude terms, and simplified using $D_\parallel=G(\chi)/v$, $D_\perp=[\operatorname{erf}(\chi)-G(\chi)]/v$, $|A|=2\Psi(\chi)/v^2$, and the identity $2\chi-\sqrt\pi e^{\chi^2}\operatorname{erf}(\chi)=-\sqrt\pi e^{\chi^2}\Psi(\chi)$. The first term's coefficient, $8T(2T-Y)$, is non-negative since $T\geq Y\geq0$ implies $2T-Y\geq T\geq0$; verified symbolically.
\end{proof}

\begin{remark}[Erratum: a factor-of-two error, found and fixed]\label{rem:erratum}
An earlier version of this note stated the first term's coefficient as $4T(2T-Y)$. This was wrong by a missing factor of two, found independently three ways: by re-deriving (5) fresh from the raw $(w_h,w_\ell,w_{hh},w_{h\ell})$ components rather than reusing the previously-typeset closed form and diffing the two symbolically (the discrepancy factored exactly onto the first term); by an external audit of this note, which located the same error independently and via a different check ($Y=0\Rightarrow w=2T$ pins the coefficient immediately); and by cross-validation against the Monte Carlo simulator of \cite{Ebert2026MonteCarloNote} at a value of $v$ small enough to make the two candidate coefficients distinguishable, which was inconclusive on its own (the extreme point tested has $\ell$ too small for the simulator's linear-response regime to resolve the difference reliably) but did not contradict the correction. All results in this note derived from (5) --- the boundary-limit propositions below, and Section~\ref{sec:secularpositivity}'s secular analysis --- have been rechecked against the corrected coefficient; where a stated result changed as a consequence, this is noted explicitly at that result.
\end{remark}
...
\begin{equation}
\langle\dot w\rangle \sim \frac{4\chi}{3\sqrt\pi}\left[\frac{T^2v}{\mu^2}(8T-3Y) + \frac{T}{\mu v}(10T-7Y)\right], \tag{7}
\end{equation}
manifestly non-negative since $Y\leq T$ gives both $8T-3Y\geq5T>0$ and $10T-7Y\geq3T>0$.
\end{proposition}

\begin{proof}
Direct symbolic series expansion of the corrected (5) in $\chi$ about $0$; the $O(\chi^0)$ and $O(\chi^2)$ terms vanish identically, leaving (7) as the leading behavior. The bracketed factorization was verified to agree with the raw series coefficient exactly (symbolic difference $0$). This coefficient differs from an earlier version of this proposition, which used the pre-erratum (5) and reported $\tfrac{4T}{\mu v}(T-Y)$ in place of $\tfrac{T}{\mu v}(10T-7Y)$ for the second bracketed piece; both are non-negative for the same reason ($Y\leq T$), so the qualitative conclusion is unchanged, but the earlier numerical coefficient should not be cited.
\end{proof}

With this, all four limits of the two-dimensional domain ($r\to0$, $e\to1$ at apoapsis, $\chi\to0$, $\chi\to\infty$) are proven safe under the corrected (5). Section~\ref{sec:secularpositivity} below reports what is now known about the interior: pointwise positivity is false, but a much stronger secular statement is established.
...
\section{Pointwise Positivity Is False; Secular Positivity Is Essentially Proven}\label{sec:secularpositivity}

Everything in Section~\ref{sec:Rminus} above concerned $\langle\dot w\rangle(\theta)$ at a single instant. This section reports a substantial correction and extension, incorporating an independently-audited external analysis of this exact problem \cite{EbertAuditedSecular2026}, whose central claims we have checked directly against our own, separately-implemented code (not merely reproduced from the source) wherever stated below.

\subsection{The pointwise conjecture is false}

Define the dimensionless variables $z:=Tv^2/\mu$, $q:=Y/T\in[0,1]$ (so $q=\sqrt{1-e^2}$), and $\Theta:=\mu/(2T\sigma^2)$ (so $\chi^2=\Theta z$). Corrected equation (5), stripped of its common positive dimensional prefactor, is
\begin{equation}
f_-(z,q,\Theta) = \frac{4(2-q)\sqrt\Theta}{\sqrt\pi}e^{-\Theta z} + \frac{G(\sqrt{\Theta z})}{\sqrt z}B(z,q,\Theta), \qquad B(z,q,\Theta):=(8-3q)z-(2+q)+4q\Theta. \tag{8}
\end{equation}

\begin{proposition}[Failure of pointwise positivity]\label{prop:pointwisefails}
$f_-\geq0$ does not hold throughout the physical domain. A confirmed violation occurs near $q\approx0.003$ ($e\approx0.9999955$), $z\approx0.103$, $\Theta\approx68.5$ (equivalently $\chi\approx2.65$), where $f_-\approx-0.094$.
\end{proposition}

\begin{proof}[Verification]
Independently confirmed here by direct evaluation of the corrected (5)/(8) at this point, using code written separately from \cite{EbertAuditedSecular2026}'s own implementation; the two agree. Since $B$ is strictly increasing in $z$ (as $8-3q>0$), any negative contribution is confined to the apoapsis side of the orbit, $z<z_c:=(2+q-4q\Theta)/(8-3q)$.
\end{proof}

This retracts Finding~\ref{find:Rminus} as originally stated. The negative pocket lies on the apoapsis side specifically, where the Kepler time weight $dt\propto r^2\,d\theta$ is \emph{largest} --- so, unlike the reversal mechanism of an earlier draft of this section (which relied on the periapsis corner being time-negligible), the surviving secular statement below is not a matter of the dangerous region being weighted away, but of a narrower, still more strongly positive apoapsis boundary layer dominating it.

\subsection{Explicit analytic regions of pointwise positivity}

\begin{proposition}[Hot/intermediate-bath region]\label{prop:hotbath}
$\Theta\leq\Theta_0:=\tfrac53\ln6\approx2.986\implies f_->0$ pointwise, for every $e$.
\end{proposition}

\begin{proposition}[Cold-bath region]\label{prop:coldbath}
$\Theta\geq(2+q)/(4q)\implies f_->0$ pointwise (this makes $B\geq0$ already at $z=0$).
\end{proposition}

\begin{corollary}[Moderate eccentricity is unconditionally safe]\label{cor:safe_ecc}
The two regions overlap for $q\geq q_*:=2/(4\Theta_0-1)\approx0.1827$, i.e.\ $e\leq\sqrt{1-q_*^2}\approx0.9832$: pointwise positivity holds at \emph{every} bath temperature for all orbits below this eccentricity.
\end{corollary}

\begin{proof}[Verification of the supporting elementary bound]
Proposition~\ref{prop:hotbath}'s proof (see \cite{EbertAuditedSecular2026}) uses the elementary bound $G(x)\leq2x/(3\sqrt\pi)$, following from $e^{-t^2}\leq1$ in $\Psi$'s integral representation; we confirmed this numerically with zero violations across $x\in[0.001,20]$, and confirmed $\Theta_0\approx2.98627$ to the stated precision.
\end{proof}

So every unresolved pointwise case is confined to the wedge $0<q<q_*$, $\Theta_0<\Theta<(2+q)/4q$ --- already a small fraction of the full $(q,\Theta)$ domain.

\subsection{The circular orbit is a strict local secular minimum}

\begin{theorem}[\cite{EbertAuditedSecular2026}]\label{thm:circularmin}
Writing $F_-(e,\Theta)=F_-(0,\Theta)+C_2(\Theta)e^2+O(e^4)$, $C_2(\Theta)>0$ for every $\Theta>0$.
\end{theorem}

\begin{proof}[Verification]
$C_2(\Theta)=\tfrac12[(26x^2-11)G(x)+(14x-24x^3)e^{-x^2}/\sqrt\pi]$, $x=\sqrt\Theta$; confirmed numerically with zero violations across $\Theta\in[10^{-4},10^3]$ (minimum value found $0.0188>0$, at the smallest $\Theta$ tested).
\end{proof}

\subsection{The secular coefficient, and independent confirmation across the dangerous wedge}

\begin{proposition}[Secular survival, \cite{EbertAuditedSecular2026}]\label{prop:secularsurvives}
At the confirmed counterexample's eccentricity and nearby, direct orbit integration finds the negative-region contribution to $F_-$ outweighed by the positive apoapsis boundary layer surrounding it (e.g.\ at $e=0.9999955,\Theta=68.47$: negative part $\approx-0.0033$, positive part $\approx4.42$, net positive).
\end{proposition}

\begin{finding}[Independent cross-check across the entire dangerous wedge]\label{find:secularverified}
Using our own orbit-averaging implementation (the same one underlying the earlier $\langle\langle4Y/v^3\rangle\rangle$ and monotonicity results below) and the corrected formula (5)/(8), $\langle\langle\dot w\rangle\rangle$ was evaluated at $42$ points spanning the full claimed dangerous wedge ($q\in\{0.005,\ldots,0.18\}$, $\Theta$ ranging from just above $\Theta_0$ to just below $(2+q)/4q$ at each $q$). Every value found was positive; the minimum was $7.73$, far from zero.
\end{finding}

\cite{EbertAuditedSecular2026} additionally reports an interval-arithmetic certification of a rigorous lower bound throughout this wedge down to $q=10^{-10}$ ($155$ parameter cells, $0$ failed, minimum certified lower functional $1.18\times10^{-5}$), via a global rational bound $G(x)\leq4x/[3\sqrt\pi(2+x^3)]$ that we confirmed numerically (zero violations, $x\in[0.001,20]$) but have not re-certified via independent interval arithmetic; that specific computation is taken on the source's authority. The source itself flags the remaining strip $0<q<10^{-10}$ as not yet closed by a uniform enclosure, though strongly controlled by an analytically positive compactified-edge limit.

\subsection{Our own orbit-averaging results, recontextualized}

The following were established before the pointwise counterexample and the analytic regions above were known, using an independent methodology (direct numerical quadrature of the time-weighted average, rather than the algebraic regions of the preceding subsections). They remain correct and are complementary --- covering the full domain numerically where the analytic regions do not yet reach, and identifying structure (the divergence mechanism, the monotonicity threshold) not visible from the analytic bounds alone.

\begin{proposition}[Both extreme-$\sigma$ limits of the orbit average are trivial]\label{prop:orbitavglimits}
As $\sigma\to0$ ($\chi\to\infty$ uniformly along the orbit), $\langle\langle\dot w\rangle\rangle\to\langle\langle4Y/v^3\rangle\rangle_{\rm time}$ (confirmed numerically to $6$ significant figures by $\sigma=10^{-4}$); since $4Y/v^3>0$ pointwise, this limit is positive with no computation required beyond Proposition~\ref{prop:chiinf}'s already-established pointwise positivity (unaffected by the erratum, since that term does not involve the corrected coefficient). As $\sigma\to\infty$, the analogous argument applies to Proposition~\ref{prop:chi0}'s corrected, still pointwise-non-negative leading term.
\end{proposition}
...
Near periapsis, $v$ is largest, so $4Y/v^3$ is smallest there, but the body also moves fastest through periapsis, down-weighting this region in the time measure $dt\propto r^2\,d\theta$. Near apoapsis, $v\to0$ as $e\to1$, so $4Y/v^3\to\infty$ there, and the body simultaneously lingers longest --- so the average is dominated by, and diverges with, apoapsis. This is consistent with, and gives an independent geometric explanation for, why Proposition~\ref{prop:secularsurvives}'s positive apoapsis boundary layer dominates the negative pocket found in Proposition~\ref{prop:pointwisefails}: both observations locate the same physical mechanism (apoapsis dominance under time-weighting) from different directions.
\end{remark}

\begin{finding}[Monotonicity in $\sigma$: true for $e\gtrsim0.5$, understood below it]\label{find:sigmamono}
$\langle\langle\dot w\rangle\rangle(\sigma)$ decreases monotonically for every $e\geq0.5$ tested, from a finite positive value as $\sigma\to0$ down to $0^+$ as $\sigma\to\infty$. Below $e\approx0.5$, a small local maximum appears (up to $8.8\%$ above the $\sigma\to0$ value at $e\to0$, shrinking continuously to $0\%$ by $e=0.5$), fully explained as inherited pointwise non-monotonicity (at $e=0$ exactly, the orbit average trivially equals the pointwise quantity): the circular-orbit pointwise hump ($8.81\%$ at $\sigma=0.316$) matches the $e=0.1$ orbit-averaged hump ($7.75\%$, same $\sigma$) almost exactly. This hump never approaches zero at any eccentricity tested.
\end{finding}

\subsection{Summary of status}

Pointwise positivity is false, with the violation confined to a specific, now-located region. Outside that region (moderate eccentricity at any temperature, or any eccentricity at sufficiently hot or cold bath temperature), pointwise positivity is proven. Inside it, the secular (orbit-averaged, physically observable) quantity is positive everywhere checked --- by two independent methods (this note's direct quadrature, and \cite{EbertAuditedSecular2026}'s algebraic regions plus interval-arithmetic certification) --- with only a strip below $q=10^{-10}$ not yet closed by a rigorous enclosure. This is a substantially stronger and better-understood result than either the original (false) pointwise claim or the earlier draft of this section's numerically-supported-but-unproven secular claim.

\section{Scope: What Remains}

\begin{itemize}
\item[--] \textbf{The never-both-negative pattern (Finding~\ref{find:neveroverlap}) is unproven,} and Proposition~\ref{prop:mechanism}'s narrowing (bounding $\operatorname{Var}(T)-\operatorname{Var}(Y)$ against the leading term $\tfrac{Y}{2}\langle\dot w\rangle$) now inherits the more nuanced status of that leading term itself: unconditionally non-negative on the explicit regions of Section~\ref{sec:secularpositivity} (moderate eccentricity, or hot/cold bath), open only in the same narrow wedge where $\langle\dot w\rangle$ itself is unresolved pointwise (though secularly safe there).
\item[--] \textbf{Finding~\ref{find:Rminus} is corrected: pointwise positivity is false, secular positivity is essentially proven.} Section~\ref{sec:secularpositivity} gives the corrected account: a genuine, located pointwise violation; explicit analytic regions (moderate eccentricity, or sufficiently hot/cold baths) where pointwise positivity holds after all; a proof that the circular orbit is a strict local secular minimum; and secular positivity confirmed, by two independent methods, throughout the remaining wedge down to $q=10^{-10}$, with only a strip below that not yet closed by a rigorous enclosure.
...
\bibitem{EbertAuditedSecular2026} J.~Ebert, \emph{Secular Positivity on the Kepler Cone: A Maxwellian-Bath Reduction and a Computer-Assisted Interior Bound}, audited working draft (2026).
\end{thebibliography}
