...
\usepackage{hyperref}
\usepackage{graphicx}
\graphicspath{{discriminant-12-return/figures/}}
...
\]
is shown throughout the figure as a concrete example.
...
\]
The sign of $-a/b$ gives the three cases. The cases $a=0$ or $b=0$ are the row/column parabolas below.
\end{proof}
...
\end{proposition}
\begin{proof}
Differentiate $Y^2=(c/b)x-(a/b)x^2$ and substitute the critical point.
\end{proof}
...
For $ab>0$, the ellipse has
\[
b_{\rm semi}=\frac{c}{2\sqrt{ab}},\qquad
a_{\rm semi}=\frac{c\sqrt{2(a^2+b^2)}}{4ab},\qquad
e=\frac{|a-b|}{\sqrt{a^2+b^2}}.
\]
Thus eccentricity depends only on $a/b$, and $e=0$ exactly when $a=b$.
\end{proposition}
\begin{proof}
The section is symmetric under $Y\mapsto-Y$. Its pure-$Y$ semi-axis is $Y_{\max}$. At $Y=0$ the endpoints are
\[
\left(\frac{c}{2a},0,\frac{c}{2a}\right),\qquad
\left(-\frac{c}{2b},0,\frac{c}{2b}\right).
\]
Their Euclidean distance gives the full major axis. Substitution into $e^2=1-b_{\rm semi}^2/a_{\rm semi}^2$ gives the stated eccentricity.
\end{proof}
...
For $n=11$ the smallest integer anti-diagonal containing the whole positive divisor region is $x+y=12$. It contains
\[
...
\]
and it meets $x+y=12$ at the two factor pairs $(1,11)$ and $(11,1)$, corresponding to $(X,T)=(-5,6)$ and $(5,6)$.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig_divisor_summatory_11_3panel.png}
\caption{The divisor summatory geometry for $n=11$ in the same three views as Figure~\ref{fig:table_line}. The $66$ cells inside $x+y\le12$ split exactly into the divisor region $D(11)=29$ and its triangular complement $A_{11}=37$. The boundary $xy=11$ is the horizontal line $Y=\sqrt{11}$ in the flat projection, the hyperbola $T^2-X^2=11$ in the side view, and the section of the cone by the vertical plane $Y=\sqrt{11}$ in three dimensions. The displayed $11\log11$ and $11H_{11}$ values are comparison quantities only; they are not asserted to equal the discrete count.}
\label{fig:divisor11}
\end{figure}

The choice $n=11$ is illustrative rather than structural: the identities above hold for general $n$. It is useful here because the extremal factor pairs $(1,11)$ and $(11,1)$ lie exactly on the outer shell $x+y=12$, making the discrete partition and the constant-product boundary visible in one compact picture.
