...
\begin{abstract}
The cutting-plane theorem of \cite{Ebert2026Table} sends a factor pair $(x,y)$ to
\[
X=\frac{x-y}{2},\qquad Y=\sqrt{xy},\qquad T=\frac{x+y}{2},
\]
so that $X^2+Y^2=T^2$, and classifies the section cut by a line $ax+by=c$ by the sign of $ab$.  We place that result inside the continuous Lorentz symmetry of the cone.  After adjoining the reflected sheet $Y<0$, every nondegenerate ellipse or hyperbola cut is a full orbit of the explicit generator
\[
G_{a,b}=(a+b)L+(a-b)B_Y\in\mathfrak{so}(2,1),
\]
whose characteristic polynomial is $\lambda(\lambda^2+4ab)$.  Thus the conic classification, the causal type of the cutting-plane normal, and the eigenvalue type of the Lorentz generator are the same invariant.  We diagonalize the elliptic and hyperbolic flows in complex and split eigen-coordinates, respectively.  Integer powers of those coordinates define semiconjugate power maps once a particular equation $(a,b,c)$ is chosen; the construction is not canonical under arbitrary rescaling of that equation.  In the parabolic boundary case the diagonal eigen-coordinate construction collapses because the generator is nilpotent, although coordinatewise factor powers $(x,y)\mapsto(x^n,y^n)$ still exist.  The same rapidity $s=\tfrac12\log(x/y)$ used here is the continuous Lorentz coordinate that later supports the discriminant-$12$ arithmetic return.
\end{abstract}
...
For positive factor coordinates define
...
y=T-X.
\end{equation}
Paper A \cite{Ebert2026Table} uses the positive-factor sheet $Y\ge0$.  The Lorentz group acts more naturally after adjoining its reflection across $Y=0$.
...
\]
with $Y$ allowed to have either sign.  The Paper-A factor image is the half-sheet
\[
...
\boxed{\langle n,n\rangle_\eta=-4ab.}
\end{equation}
Hence:
\begin{enumerate}[label=(\roman*)]
\item $ab>0$: $n$ is timelike and the nonzero eigenvalues are $\pm2i\sqrt{ab}$;
\item $ab<0$: $n$ is spacelike and the nonzero eigenvalues are $\pm2\sqrt{-ab}$;
\item $ab=0$: $n$ is null and $G_{a,b}$ is nilpotent.
\end{enumerate}
...
\[
(a,b)\mapsto(\kappa a,\kappa b)
\quad\Longrightarrow\quad
G_{\kappa a,\kappa b}=\kappa G_{a,b}.
\]
Thus the underlying one-parameter subgroup is naturally attached to the projective direction $[a:b]$, while the matrix generator is fixed only up to time rescaling.

\section{Signed cuts are complete Lorentz orbits}

\begin{theorem}[Orbit theorem]\label{thm:orbit}
Assume $ab\ne0$ and that the plane \eqref{eq:plane} meets the future cone in a nondegenerate section.  On the signed future cone $\mathcal C^+$, each connected nondegenerate section is a full orbit of the one-parameter subgroup $e^{tG_{a,b}}$.  The multiplication-table trace of Paper A is the portion of that orbit lying on $Y\ge0$.
\end{theorem}

\begin{proof}
Proposition~\ref{prop:invariant} shows that every orbit is contained in the plane section.  If $ab>0$, Proposition~\ref{prop:classification} gives an elliptic one-parameter subgroup; after a Lorentz change of frame sending the timelike plane normal to the time axis, the section is a Euclidean circle and the subgroup is an ordinary rotation, hence transitive on it.  If $ab<0$, the normal is spacelike; after a Lorentz change of frame sending it to a spatial axis, the section is the future branch of a hyperbola in a timelike two-plane and the subgroup is an ordinary boost, again transitive on that connected branch.  Returning by the inverse Lorentz change of frame proves the claim.  Restricting to $Y\ge0$ gives the Paper-A factor trace.
\end{proof}

\begin{remark}
The signed-sheet qualification is essential.  For $a=b$, $G_{a,a}=2aL$ rotates the full signed circle, whereas the factor map $Y=+\sqrt{xy}$ displays only its upper semicircle.  Likewise, $a=-b$ gives the full signed $B_Y$ hyperbola, while the factor sheet retains only $Y\ge0$.
\end{remark}
...
Thus the ellipse becomes a circle in its own natural eigen-coordinate.
...
The two cases therefore have parallel normal forms:
\[
|\zeta|^2=c^2
\qquad\text{or}\qquad
\eta_+\eta_-=c^2.
...
A geometric line is unchanged by
\[
(a,b,c)\mapsto(\kappa a,\kappa b,\kappa c),
\qquad \kappa\ne0.
\]
The power construction below is therefore attached to a \emph{chosen equation} $(a,b,c)$, not canonically to the unnormalized geometric line.  This distinction is necessary: the eigen-coordinates scale linearly with the coefficients, while their $n$th powers scale by $\kappa^n$.
...
\begin{proposition}[Elliptic power map]\label{prop:ellpower}
...
\begin{proposition}[Hyperbolic power map]\label{prop:hyppower}
...
For either nonparabolic eigen-coordinate construction,
...
\]
The right-hand side is exactly the time-$nt$ flow starting from the output point $\Psi_n(z_0)=z_0^n$.  The split case is the same calculation on the pair $(\eta_+,\eta_-)$ with opposite exponents.
\end{proof}

\begin{remark}[Normalization guardrail]
If the same geometric line is rewritten as $(\kappa a)x+(\kappa b)y=\kappa c$, then the chosen eigen-coordinate is rescaled before taking its $n$th power.  For $n>1$ this changes the target geometric level.  Thus \eqref{eq:semiconj} is exact for a fixed chosen equation, but the map is not invariant under arbitrary rescaling of that equation.
\end{remark}
...
For $ab=0$, there is no nonzero diagonal eigen-coordinate of the elliptic/hyperbolic type used in Sections 5--6.  Consequently the preceding eigen-coordinate power-map construction does not extend by continuity as a diagonalization argument.
\end{proposition}
...
\boxed{s\mapsto ns.}
\end{equation}
Thus coordinatewise powers dilate Lorentz rapidity while raising the product level.  This is structurally different from the chosen-equation eigen-coordinate powers of Sections 6, even though both are naturally expressed in coordinates adapted to one-parameter Lorentz actions.
...
The continuous symmetry behind Paper A is now explicit.  The cutting-plane normal has Lorentz norm $-4ab$, the corresponding generator has characteristic polynomial $\lambda(\lambda^2+4ab)$, and the signed nondegenerate section is a full orbit of that generator.  Complex and split eigen-coordinates reduce the elliptic and hyperbolic cases to $|\zeta|^2=c^2$ and $\eta_+\eta_-=c^2$.  Integer powers give exact semiconjugate maps once a particular equation $(a,b,c)$ is fixed, while the parabolic boundary marks the failure of diagonalization rather than the failure of all power operations.  Together with the common rapidity variable, these results make the present paper the continuous Lorentz-symmetry companion to the elementary geometry of Paper A and the later arithmetic return construction.
