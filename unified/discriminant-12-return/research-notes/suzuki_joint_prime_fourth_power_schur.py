#!/usr/bin/env python3
"""Fourth-power Schur audit for Suzuki's joint prime operator at a=1.
...
nonnegative symmetric kernel,
    ...
    ||B_prime|| = ||A|| <= ( sup_x (A^4 1)(x) )^(1/4).

For piecewise-constant inputs, A preserves piecewise constancy.  Starting from
1 on [-1,1], the breakpoint set at each iterate is generated exactly by
shifting the previous breakpoints by +/- log(q) and retaining points inside
[-1,1].  Thus sup_x(A^4 1)(x) reduces to a finite exhaustive interval sweep.

The floating-point evaluation gives

    sup_x(A^4 1)(x) = 25.69449057762533,
    ||B_prime|| <= 2.2514380572859216.

This improves the v13.307 square-Schur bound 2.460536122083583.  The reduction
to a finite breakpoint maximum is exact; the displayed decimal is ordinary
floating-point evaluation rather than interval arithmetic.

Combining this constant with the v13.304 analytic archimedean bound and the
tail-localized cusp/pole estimates gives first sufficient odd cutoff N=1433.
...
SHIFTS = np.array([math.log(2.0), math.log(3.0), math.log(4.0),
...
def power_schur(power: int = 4) -> dict:
    ...
    for k in range(1, power+1):
        ...
        seq.append({'power': k, 'intervals': len(vals), 'sup_Ak1': supv,
...
def coercive_cutoff(max_search: int = 100000) -> dict:
    prime = power_schur(4)['prime_bound']
    for N in range(1, max_search+1, 2):
        total = math.pi/2.0 + prime + arch_remainder_analytic_bound() + \
...
if __name__ == '__main__':
    ...
    for row in power_schur(4)['sequence']:
        print(row)
