...
stages. Source uncertainty is absorbed by the shifted nominal operator
A_nom-2e-13 I. All transcript radii below are rounded outward.
"""
...
# Stage 1: 7991-dimensional shifted LDL/Woodbury solve.
# u=5.421010862427522e-20, gamma_7991=4.331929780165835e-16.
SOLVE_RES_FROB=D('6.953589059005369e-15')
SOLVE_DOT_ROUND_FROB=D('2.135044237583902e-15')
SOLVE_RES_TOTAL=D('9.08863329658927e-15')

# Stages 1+2: residual/coercivity propagation plus finite Schur and Q/N
# formation. Raw formation envelopes were <3.34e-9, <4.1e-13, <6.02e-17
# on QQ, QN, NN. Projected solve-residual propagation gives the rounded totals:
Q_FINITE_RAD=D('2.0e-8')
QN_FINITE_RAD=D('7.0e-13')
NN_FINITE_RAD=D('1.0e-16')

# Stage 3: explicit remote Gram 16003..2,000,000. These enlarged radii include
# gamma_n chunk accumulation, inverse-power moment formation, and propagation
# of the finite-solve residual into W=[Basis;-X Basis]. They are intentionally
# much larger than the direct double/long-double discrepancies
# (QQ 9.67e-18, QN 3.84e-23, NN 1.64e-29).
QQ_REMOTE_GRAM_RAD=D('1e-10')
QN_REMOTE_GRAM_RAD=D('1e-16')
NN_REMOTE_GRAM_RAD=D('1e-20')

# Stage 4: blockwise analytic far tail. Midpoints:
# fQ=4.706301536488523e-4, fN=1.2657888576676901e-15.
# Double/long-double fN difference is <9e-26. The following include moment
# gamma_8001 accounting and the same W-perturbation allowance.
QQ_FAR_GRAM_RAD=D('1e-10')
QN_FAR_GRAM_RAD=D('1e-16')
NN_FAR_GRAM_RAD=D('1e-20')
