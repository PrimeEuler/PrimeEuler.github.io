...
def main(chunk=128):
    ...
    h=high.astype(LD); z=ZF.astype(LD); cc=cF.astype(LD); xd=X.astype(LD); rhs=AFC.astype(LD); dg=diag0[10:].astype(LD); pi=LD(PI)
    ...
    for s in range(0,N,chunk):
        ...
        with np.errstate(divide='ignore',invalid='ignore'): A=-(LD(2)/pi)*(n*z[s:e,None]-m*z[None,:])/(n*n-m*m)
        ...
        for r in range(10):
            ...
            pole=2*cc[s:e]*cx[r]; er=dr+2*np.abs(cc[s:e])*cx_rad[r]+gamma(3)*(np.abs(sm)+np.abs(pole)); val=sm+pole
            er+=gamma(1)*(np.abs(val)+np.abs(rhs[s:e,r])); R[:,r]=val-rhs[s:e,r]; E[:,r]=er
    ...
    hi=outward_frobenius(R,E); print('CERTIFIED-ARITHMETIC-STAGE RESIDUAL <=',hi)
