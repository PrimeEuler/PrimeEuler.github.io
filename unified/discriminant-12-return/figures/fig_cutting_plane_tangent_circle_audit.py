...
# Controlled audit variant of unified/fig_cutting_plane_3panel.py.
#
# Purpose:
#   1. draw EVERY anti-diagonal shell K=1,...,12, hence every
#      fixed-T circle T=K/2 at half-unit radius spacing;
#   2. emphasize the exact parabola/circle tangent family for
#      u=5,6,7, whose tangent-circle radii are 5/2, 3, 7/2;
#   3. remove circular text bounding boxes around multiplication-table
#      labels so annotation glyphs cannot be mistaken for geometry.
#
# Exact theorem being tested visually:
#   fixed row/column level u is tangent in the flat (X,Y) projection
#   to X^2 + Y^2 = (u/2)^2 at the parabola vertex X=+-u/2, Y=0.
...
# Full anti-diagonal circle family: K=1,...,12 -> T=K/2.
...
# Explicit labels for the three exact tangent circles.
...
# Row and column parabola families, clipped at x+y=12.
for xv in range(1, Kmax):
...
# Exact tangent points for u=5,6,7: vertices of the mirror parabolas.
for u in TANGENT_LEVELS:
    Tc = u / 2.0
...
# Multiplication-table labels WITHOUT circular bounding boxes.
...
# Original x+y=8 shell remains highlighted for comparison.
axA.plot(
    4 * np.cos(theta_c),
    4 * np.sin(theta_c),
    color="#24527a",
...
for curve in curves:
    axA.plot(
        curve["X"], curve["Y"],
        color=curve["color"], lw=2.4, zorder=6, label=curve["label"]
    )
    axA.plot(
        curve["X"], -curve["Y"],
        color=curve["color"], lw=2.4, zorder=6
    )
...
# =================================================================
# Panel (b): side view (X,T), showing the same half-step shell family.
# =================================================================
...
# Every K shell is a horizontal T=K/2 slice. Draw faint segments.
for K in range(1, Kmax + 1):
    Tc = K / 2.0
    is_tangent_level = K in TANGENT_LEVELS
    axB.plot(
...
# Row/column generator mesh at integer factor spacing.
for k in range(0, Kmax + 1):
    axB.plot(
        [k / 2.0, k - Rmax if k - Rmax <= k / 2.0 else k / 2.0],
...
# Plain multiplication-table labels, no circular boxes.
for xv, yv in table_pts:
    Xl = (xv - yv) / 2.0
    Tl = (xv + yv) / 2.0
    axB.text(
...
# Tangent points in side view are on the cone generators:
# (+-u/2, u/2), corresponding to Y=0.
for u in TANGENT_LEVELS:
    Tc = u / 2.0
...
# Original x+y=8 slice and cutting planes.
axB.plot(
    [-4, 4], [4, 4],
    color="#24527a", lw=2.3, ls="--", zorder=5,
    label=r"$x+y=8$",
)
...
for curve in curves:
    ...
    axB.plot(
        Xb[validb], Tb[validb],
        color=curve["color"], lw=2.4, zorder=5, label=curve["label"]
    )
axB.plot(
    [Xstar], [Tstar], "^",
    color="#b3211a", ms=7, zorder=6,
...
axC.plot_surface(
    Xg, Yg, Tg_,
    color="0.85", alpha=0.22, linewidth=0, antialiased=True, zorder=1
)
...
# Mark the exact u=5,6,7 tangent points at Y=0 on both generators.
for u in TANGENT_LEVELS:
    Tc = u / 2.0
    axC.scatter([-Tc, Tc], [0, 0], [Tc, Tc], s=20, color="#111111", depthshade=False)

# Original K=8 circle highlighted for comparison.
axC.plot(
    4 * np.cos(theta), 4 * np.sin(theta), 4,
    color="#24527a", lw=2.0, ls="--", zorder=4
)

for curve in curves:
    axC.plot(curve["X"], curve["Y"], curve["T"], color=curve["color"], lw=2.6, zorder=5)
    axC.plot(curve["X"], -curve["Y"], curve["T"], color=curve["color"], lw=2.6, zorder=5)
    ...
    axC.plot_surface(
        Xp_, Yp_, Tp_,
        color=curve["color"], alpha=0.16, linewidth=0, zorder=3
    )
...
fig.savefig(
    output_dir / "fig_cutting_plane_tangent_circle_audit.pdf",
    dpi=220,
    bbox_inches="tight",
    facecolor="white",
)
fig.savefig(
    output_dir / "fig_cutting_plane_tangent_circle_audit.png",
    dpi=220,
    bbox_inches="tight",
    facecolor="white",
)
