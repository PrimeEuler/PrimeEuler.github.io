# Cone Derivation Ledger v13.430 — V4 All-Character Exponential Transform and Mod-24 Unit-Core Lift

**Renumbering note:** originally filed as `v13.429`, which collided with `Cone_Derivation_Ledger_v13.429_External_Audit_Round_30.md`. Renumbered to `v13.430` after a live ledger recheck on 2026-09-14; mathematical content unchanged.
...
\mathcal G_\chi(n,t)
=
\sum_{k\le n}\chi(k)\left(e^{t\{n/k\}}-1\right).
\]
...
\[
\boxed{\mathbf G(n,t)=H_4\mathbf F(n,t)},
\qquad
\boxed{\mathbf F(n,t)=\frac14H_4\mathbf G(n,t)}.
\]
...
\mu_r^{(n)}
=
\sum_{\substack{k\le n\\k\equiv r\pmod{12}}}\delta_{\{n/k\}}
\]
...
\boxed{\boldsymbol\nu=H_4\boldsymbol\mu},
\qquad
\boxed{\boldsymbol\mu=\frac14H_4\boldsymbol\nu}.
\]

The exponential transforms are simply

\[
\mathcal F_r(n,t)=\int(e^{tx}-1)\,d\mu_r^{(n)}(x),
\qquad
\mathcal G_\chi(n,t)=\int(e^{tx}-1)\,d\nu_\chi^{(n)}(x).
\]
...
\[
M_{j,r}(n)=\sum_{\substack{k\le n\\k\equiv r\pmod{12}}}\{n/k\}^j,
\qquad
M_{j,\chi}(n)=\sum_{k\le n}\chi(k)\{n/k\}^j.
\]

Differentiating at t=0 gives

\[
\boxed{\mathbf M_{j,\chi}=H_4\mathbf M_{j,r}},
\qquad
\boxed{\mathbf M_{j,r}=\frac14H_4\mathbf M_{j,\chi}}.
\]
...
\mathcal F_r^{(2)}(n,t)
=
\sum_{\substack{k\le n\\k\equiv2r\pmod{24}}}
\left(e^{t\{n/k\}}-1\right),
\]
...
\chi(k/2)\left(e^{t\{n/k\}}-1\right),
\]

with the same exact transform

\[
\boxed{\mathbf G^{(2)}=H_4\mathbf F^{(2)}},
\qquad
\boxed{\mathbf F^{(2)}=\frac14H_4\mathbf G^{(2)}}.
\]
...
H_4\mathbf m
\approx
(1.0000000,\ 0.1843380,\ -0.6392576,\ -0.2770080)^T.
}
\]
...
\xleftrightarrow{\ H_4\ }
\text{four V4 character coordinates}.
}
\]
