# Cone Project — Derivation Ledger v13.170

**Version:** 13.170 — audited consolidation continuation  
**Date:** 2026-09-03  
**Consolidates:** v13.127 audited baseline together with the audited/corrected developments through v13.170.  
...
# PART XXXIII. PAPER-A FIGURE SOURCE RECOVERY

## 136. Stale generator identified

**[Audit]** The repository copy of `fig_cutting_plane_3panel.py` did not
generate Figure 1 of Paper A v2.  It still used the single cut

\[
2x+5y=20
\]

and its maximum at \((x,y)=(5,2)\), whereas the published v2 figure and caption
use the reflected pair
...
with the red-cut maximum at

\[
(x,y)=(2,4).
\]

Thus the paper PDF was current but its committed generator was stale.

## 137. Recovered v2 construction

**[D/computationally reproducible]** The generator now maps both positive
linear cuts through
...
For the red cut \(8x+4y=32\), the edge-on equation is

\[
-4X+12T=32,
\]

and for its purple reflection \(4x+8y=32\),

\[
4X+12T=32.
\]

The two ellipses are therefore reflections across \(X=0\).  The maximum of
\(Y^2=xy\) on the red cut occurs at
...
These are the values now used by the script and displayed in the legend.

## 138. Visual layers restored

The recovered generator reproduces the Paper A v2 figure architecture:

1. concentric even-\(K\) circles through \(K=12\);
2. the complete multiplication-table labels for \(x+y\le12\);
3. both row and column parabola meshes;
4. the dashed \(x+y=8\) circle/side-view segment;
5. the red and purple reflected ellipses;
6. both edge-on cutting lines in the \((X,T)\) view;
7. both translucent cutting planes on the three-dimensional cone;
8. the shell labels \(K=2,4,6,8,10,12\) in the side view.

The output is written portably beside the script as both
`fig_cutting_plane_3panel.pdf` and `fig_cutting_plane_3panel.png`.

## 139. Repository role

The corrected script and both generated assets belong in two places:

- the parent `unified/` directory, where Paper A v2 expects the figure PDF;
- the discriminant-12 project’s `foundations/` directory, so its Paper A
  snapshot is independently compilable and retains the source of its main
  figure.

No mathematical text or compiled Paper A artifact is changed by this source
recovery.

## 140. v13.170 checkpoint

Paper A v2 now has a reproducible main figure whose source matches the
published geometry:

\[
\boxed{
\begin{array}{c}
x+y=8\quad\text{(fixed-radius comparison)},\\
8x+4y=32\quad\text{(red ellipse)},\\
4x+8y=32\quad\text{(purple reflected ellipse)},\\
x+y\le12\quad\text{(multiplication-table boundary)}.
\end{array}
}
\]

The recovery is editorial and computational, not a new theorem.  Its value is
that the visual foundation used by Paper A and the new mod-\(12\) shell can now
be rebuilt directly from the repository.

---

**End of v13.170 audited consolidation continuation.**
