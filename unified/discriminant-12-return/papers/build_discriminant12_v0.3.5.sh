...
FIG="$HERE/../figures"
SRC="Discriminant_12_Return_v0.3.5.tex"
BUILD="$HERE/.build_discriminant12_v0.3.5"
STAGED="$BUILD/Discriminant_12_Return_v0.3.5.tex"
...
printf '==> compiling isolated Discriminant-12 v0.3.5 build\n'
cd "$BUILD"
TEXINPUTS=".:$FIG:$HERE:" pdflatex -interaction=nonstopmode -halt-on-error "$(basename "$STAGED")"
TEXINPUTS=".:$FIG:$HERE:" pdflatex -interaction=nonstopmode -halt-on-error "$(basename "$STAGED")"

cp "$BUILD/Discriminant_12_Return_v0.3.5.pdf" "$HERE/Discriminant_12_Return_v0.3.5.local-build.pdf"

printf '==> build complete: %s\n' "$HERE/Discriminant_12_Return_v0.3.5.local-build.pdf"
