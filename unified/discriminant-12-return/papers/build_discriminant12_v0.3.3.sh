...
FIG="$HERE/../figures"
SRC="Discriminant_12_Return_v0.3.3.tex"
BUILD="$HERE/.build_discriminant12_v0.3.3"
STAGED="$BUILD/Discriminant_12_Return_v0.3.3.tex"
...
printf '==> compiling isolated Discriminant-12 v0.3.3 build\n'
cd "$BUILD"
TEXINPUTS="$HERE:$FIG:" pdflatex -interaction=nonstopmode -halt-on-error "$(basename "$STAGED")"
TEXINPUTS="$HERE:$FIG:" pdflatex -interaction=nonstopmode -halt-on-error "$(basename "$STAGED")"

cp "$BUILD/Discriminant_12_Return_v0.3.3.pdf" "$HERE/Discriminant_12_Return_v0.3.3.local-build.pdf"

printf '==> build complete: %s\n' "$HERE/Discriminant_12_Return_v0.3.3.local-build.pdf"
