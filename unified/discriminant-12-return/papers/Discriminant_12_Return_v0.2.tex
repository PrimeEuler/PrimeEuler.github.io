...
\section{The geometric mod-$12$ unit shell}
\label{sec:mod12-shell}

The arithmetic selected above also has a direct realization in the factor-cone
coordinates of Paper A.  The unit residues modulo $12$ are
\[
U(12)=(\Z/12\Z)^\times=\{1,5,7,11\}.
\]
For each $r\in U(12)$, take the factor pair $(r,1)$ together with its mirror
$(1,r)$.  Under
\[
T=\frac{x+y}{2},
\qquad
...
\end{array}
\]
The outer pair $r=11$ is the factor pair $(11,1)$ and its mirror $(1,11)$;
it therefore lies on the Paper-A anti-diagonal $x+y=12$, or equivalently the
fixed-radius cone circle $T=6$.  The remaining units occupy the nested circles
$T=1,3,4$ along the same distinguished row/column pair $x=1$ and $y=1$.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{mod12_v4_cone_triple.png}
\caption{The mod-$12$ unit shell in factor-cone coordinates.  For each
$r\in\{1,5,7,11\}=U(12)$, the factor pairs $(r,1)$ and $(1,r)$ map to
$T=(r+1)/2$, $X=\pm(r-1)/2$, and $Y=\sqrt r$, so that
$T^2-X^2=r$.  Panel (a) is the flat $(X,Y)$ projection, panel (b) is the
$(X,T)$ side view, and panel (c) shows the corresponding curves on the null
cone.  The outer $r=11$ pair lies on $x+y=12$.  The highlighted labels form
$V_4$ under multiplication modulo $12$; the cone embedding realizes their
geometry but does not replace the modular group law.}
\label{fig:mod12-unit-shell}
\end{figure}

\begin{proposition}[Mod-$12$ Klein group]
Every element of $U(12)$ has order dividing $2$, and
\[
\boxed{U(12)\cong C_2\times C_2=V_4.}
\]
Under the cyclotomic identification used below, $r\in U(12)$ acts by
\[
\zeta_{12}\longmapsto\zeta_{12}^{,r},
\]
so the same four labels index
$\Gal(\Q(\zeta_{12})/\Q)$.
\end{proposition}
...
\]
The group has four elements and three nonidentity involutions, hence is
$C_2\times C_2$.  The Galois statement is the standard cyclotomic
identification $\Gal(\Q(\zeta_n)/\Q)\cong(\Z/n\Z)^\times$ at $n=12$.
\end{proof}

\begin{remark}[Layer boundary]
Figure~\ref{fig:mod12-unit-shell} is a geometric carrier for the four residue
labels.  Neither proximity nor adjacency in the drawing defines their
multiplication.  Moreover, the modulus $12$, the characteristic discriminant
$\Delta_{g_{12}}=12$, the cyclotomic Galois $V_4$, and the Boolean translation
$V_4^{\rm tr}$ remain logically distinct.  Their agreement in the selected
return is an exact compatibility developed in this paper, not a definitional
identity.
\end{remark}
